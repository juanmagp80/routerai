...
-- Esta versión elimina funciones existentes primero para evitar conflictos

-- Eliminar funciones existentes para evitar conflictos de tipos
DROP FUNCTION IF EXISTS get_user_learning_stats(TEXT);
DROP FUNCTION IF EXISTS get_recommended_models(TEXT, TEXT, INTEGER);
DROP FUNCTION IF EXISTS upsert_model_usage(TEXT, TEXT, TEXT, INTEGER, DECIMAL, INTEGER, BOOLEAN, BOOLEAN, TEXT);
DROP FUNCTION IF EXISTS calculate_model_score(TEXT, TEXT, TEXT);
...
DECLARE
    ...
    models_data JSON;
BEGIN
    -- Obtener datos ordenados de modelos
    SELECT json_agg(
        json_build_object(
    ...
    FROM (
        SELECT model_name, provider, usage_count, success_rate, total_cost, average_response_time
    ...
    SELECT json_build_object(
        ...
        'models_by_usage', COALESCE(models_data, json_build_array())
    ) INTO result
    FROM user_model_preferences
    WHERE user_id = p_user_id;
